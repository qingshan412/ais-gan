...
class DCGAN(object):
  ...
  def discriminator(self, image, y=None, reuse=False):
    with tf.variable_scope("discriminator") as scope:
      ...
      if not self.y_dim:
        ...
        h0 = lrelu(bconv2d(x, self.c_dim + self.y_dim, name='d_h0_bconv'))
        h0 = conv_cond_concat(h0, yb)

        h1 = lrelu(self.d_bn1(bconv2d(h0, self.df_dim + self.y_dim, name='d_h1_bconv')))
  ...
  def classifier(self,image1, y=None):
    with tf.variable_scope("discriminator") as scope:
      scope.reuse_variables()
    if not self.y_dim:
      h0 = conv2d_d1(image1, self.df_dim, name='d_h0_conv')
      h1 = self.d_bn1(conv2d_d2(lrelu(h0), self.df_dim, name='d_h1_conv'))
      h2 = self.d_bn2(conv2d_d1(lrelu(h1), self.df_dim*2,name='d_h2_conv'))
      h3 = self.d_bn3(conv2d_d2(lrelu(h2), self.df_dim*2, name='d_h3_conv'))
      h4 = self.d_bn4(conv2d_d1(lrelu(h3), self.df_dim*4,name='d_h4_conv'))
      h5 = self.d_bn5(conv2d_d2(lrelu(h4), self.df_dim*4, name='d_h5_conv'))
  ...
  def save(self, checkpoint_dir, step):
    ...
    #        global_step=step)
    # J.L. 
    #self.saver.export_meta_graph(self.sess,
    #        os.path.join(checkpoint_dir, model_name + '.meta'))
    self.saver.save(self.sess, 'my-save-dir/my-model-10000')
      # Generates MetaGraphDef.
    self.saver.export_meta_graph('my-save-dir/my-model-10000.meta')
